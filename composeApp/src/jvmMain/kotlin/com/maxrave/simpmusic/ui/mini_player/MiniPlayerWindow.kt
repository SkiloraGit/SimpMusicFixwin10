...
fun MiniPlayerWindow(
    ...
    // BUGFIX: mirror the same transparent-mode detection as DesktopApp so that the
    // mini-player is also visible on Windows 10 machines where transparent/undecorated
    // windows fail to render (invisible ghost window).
    val useTransparent = remember {
        val osName = System.getProperty("os.name", "")
        if (!osName.contains("Windows", ignoreCase = true)) return@remember true
        // Honour the same user override flag file and system property as DesktopApp
        ...
        if (flagFile?.exists() == true) return@remember false
        true
    }
